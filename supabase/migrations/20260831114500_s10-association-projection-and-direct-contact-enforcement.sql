-- S10: Trusted project-contact association projection and direct-contact enforcement.
-- Reference: dev-docs/specs/s10/s10-01-production-readiness-foundation-and-task-detail-implementation-spec.md
--
-- Forward-only repair. The S10 administration UI must never read
-- project_client_contacts directly. Associations represent direct-contact
-- readiness metadata only; organization contacts use organization matching and
-- cannot be newly associated.

begin;

-- Preserve the existing Admin/PM-only command surface, but reject new
-- organization-contact associations. Disassociation remains available for every
-- existing association so historical invalid rows can be remediated safely.
...
begin
  ...
  if p_associated and v_contact.client_id is not null then
...
-- Purpose-limited administration projection. It intentionally returns only
-- contact IDs, so the existing authorized directory projection remains the sole
-- source of contact display data.
