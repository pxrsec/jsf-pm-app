-- S10-04 directory keyset-cursor projection repair.
--
-- The S10-04 public directory contract orders profiles by
-- (profiles.created_at DESC, profiles.id DESC), and application pagination is
-- required to continue from the 25th visible row. The prior RPC exposed user_id
-- but not the corresponding created_at sort component, making a composite cursor
-- impossible to construct without an unsafe second lookup or client inference.
--
-- This forward migration exposes only the existing ordering value as created_at.
-- It preserves manager authorization, row scope, result ordering, limits, all
-- existing fields, and the authenticated-only execute posture.
