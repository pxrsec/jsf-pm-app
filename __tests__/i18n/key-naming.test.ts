    ...
    for (const key of allKeys) {
      for (const pattern of forbiddenPatterns) {
        ...
        for (const seg of segments) {
          if (pattern.test(seg) && !/^(brandName|brand|name|title|label|description|notice|placeholder|action|submit|confirm|cancel|close|open|save|edit|delete|view|list|create|update|remove)$/i.test(seg)) {
            // This is a warning pattern, not a hard fail - but we enforce semantic naming
            throw new Error(`RED: Key "${key}" contains visual/position/route coupling in segment "${seg}" (matches ${pattern})`);
          }
        }
      }
    }
