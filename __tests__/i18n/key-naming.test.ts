...
describe("VC-I18N-008: Translation keys follow semantic naming convention", () => {
  ...
  it("keys are namespaced under shell, privacy, errors, auth, theme, projects, notifications, or notificationOperations", () => {
    function collectKeys(obj: Record<string, unknown>, prefix = ""): string[] {
      return Object.entries(obj).flatMap(([k, v]) => {
        const fullKey = prefix ? `${prefix}.${k}` : k;
        if (typeof v === "object" && v !== null && !Array.isArray(v)) {
    ...
    for (const key of allKeys) {
      expect(key).toMatch(
        /^(shell|privacy|errors|auth|theme|projects|notifications|notificationOperations)\./,
      );
    }
  });
