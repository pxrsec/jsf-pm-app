...
describe("VC-I18N-008: Translation keys follow semantic naming convention", () => {
  ...
  it("keys are namespaced under valid domain namespaces", () => {
    ...
    for (const key of allKeys) {
      expect(key).toMatch(
        /^(shell|privacy|errors|auth|theme|projects|notifications|notificationOperations|calendar|archive|linkIncidents|metrics|adminOperations|clientAdministration|operationalLifecycle|accountAccess)\./,
      );
    }
  });
