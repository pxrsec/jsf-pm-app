...
import { Briefcase, Building2, ChevronRight, UserCheck } from "lucide-react";
...
interface PmCreateFormProps {
  clients: ClientListItem[];
}

export function PmCreateForm({ clients }: PmCreateFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    router.push(`/pm/proyectos/${res.data.id}`);
  };
      ...
      <Card className="border-border bg-card">
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* PM Auto Assignment Note */}
            <div className="flex items-start gap-2.5 p-3 rounded-lg border border-indigo-500/20 bg-indigo-500/5 text-xs text-indigo-900 dark:text-indigo-200">
              <UserCheck className="h-4 w-4 shrink-0 text-indigo-600 dark:text-indigo-400 mt-0.5" />
              <span>{tForm("pmAutoAssignedNote")}</span>
            </div>
