...
function TaskList({
  ...
  tasks: readonly MilestoneTaskDto[];
  userRole: MilestoneDetailDialogProps["userRole"];
}) {
  ...
  return (
    <section className="space-y-3" aria-labelledby="milestone-related-tasks">
      <h3
        id="milestone-related-tasks"
        className="text-sm font-semibold text-foreground"
      >
        {t("relatedTasks")}
      </h3>
      {tasks.length === 0 ? (
        <p className="text-sm text-muted-foreground">{t("untracked")}</p>
      ) : (
            ...
            <li key={task.taskId}>
              ...
              >
                ...
                <span className="mt-1 block text-xs text-muted-foreground">
                  {task.projectName} ·{" "}
                  {tStatus(
                    `taskStatus.${statusKeys[task.status] ?? "pending"}`,
                  )}
                </span>
              </Link>
            </li>
...
function MilestoneDetailContent({
  ...
  return (
    <DialogContent className="max-h-[calc(100dvh-1rem)] w-[calc(100%-1rem)] overflow-y-auto sm:max-w-2xl">
      <DialogHeader>
      ...
      {detail && (
        <div className="space-y-5">
          <dl className="grid gap-3 sm:grid-cols-2">
            <DetailField label={t("detail.milestoneTitle")}>
              {detail.title}
            </DetailField>
            <DetailField label={t("detail.milestoneType")}>
              {detail.scope === "company"
                ? t("scope.companyMilestone")
                : t("scope.projectMilestone")}
            </DetailField>
            <DetailField label={t("detail.targetDate")}>
              {targetDate}
            </DetailField>
            {detail.projectName && (
            ...
            {detail.description && (
              <div className="sm:col-span-2">
                <DetailField label={t("detail.descriptionLabel")}>
                  {detail.description}
                </DetailField>
              </div>
            )}
          ...
          {progress === null ? (
            ...
            <section className="space-y-2" aria-labelledby="milestone-progress">
              <h3
          ...
          <TaskList tasks={detail.tasks} userRole={userRole} />
        </div>
      )}
      <DialogFooter>
        {canManage && detail && (
