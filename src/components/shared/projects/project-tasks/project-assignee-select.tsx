...
export interface ProjectAssigneeSelectProps {
...
export function ProjectAssigneeSelect({
...
}: ProjectAssigneeSelectProps) {
  const [open, setOpen] = useState(false);
  const tProjects = useTranslations("projects.members.capacities");

  // Filter only active, non-deleted members matching allowed capacities
  const compatibleMembers = members.filter(
    (m) =>
      !m.deleted_at &&
      m.profile &&
      m.profile.is_active &&
  ...
  const selectedMember = compatibleMembers.find((m) => m.user_id === value);
  ...
  return (
    <div className="space-y-1.5">
      ...
      {compatibleMembers.length === 0 ? (
        <div className="flex items-center h-10 px-3 rounded-md border border-dashed border-border bg-muted/30 text-xs text-muted-foreground">
          {noCompatibleMembersText}
        </div>
      ) : (
        <Popover open={open} onOpenChange={setOpen}>
          ...
          <PopoverContent className="w-[320px] p-0" align="start">
            <Command>
              ...
              <CommandList>
                <CommandEmpty>{noCompatibleMembersText}</CommandEmpty>
                <CommandGroup>
